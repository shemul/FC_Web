(function (namespace, $) {
	...
	p._initStateToastr = function () {
		...
		$('#updateThisPersons').on('click', function (e) {
			o._toastrStateConfig();
	...
	p._initPositionToastr = function () {
		...
		$('.position-toast').on('click', function (e) {
			...
			toastr.success('Person has been updated !', '');
		});

		$('.position-toast-delete').on('click', function (e) {
			toastr.options.hideDuration = 0;
			toastr.clear();
			o._toastrStateConfig();
			toastr.options.timeOut = 3000;
			toastr.options.positionClass = $(e.currentTarget).data('position');
			toastr.success('Person has been Deleted !', '');
		});
	};
