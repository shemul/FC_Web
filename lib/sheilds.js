...
socket.emit('ask_battery_status','whats up');
socket.on('here_you_go_battery', function(msg){
  batteries = msg ;
  console.log(msg);
});

socket.emit('get_sheilds','ss');
socket.on('get_sheild', function(msg){

global_data = msg.item[0];
var html = '';
msg = msg.item[0];
...
for (var i = msg.length - 1; i >= 0; i--) {
      ...
      html+= "<div class='col-xs-12 col-lg-6 hbox-xs' id="+msg[i].f_imei +">"
          html+="<div class='hbox-column width-2 imageItem'>"
               html+='<img class="img-circle img-responsive imgView" src="assets/img/sheild_icon.png" alt="" />'
          html+="</div>"

          html+='<div class="hbox-column v-top">'
            html+="<div class='clearfix'>"
              html+='<div class="col-lg-12 margin-bottom-lg">'
                html+="<a class='text-lg text-medium' href='#'>Tab "+  i + "</a>"
              html+="</div>"
            html+="</dib>"
            html+='<div class="clearfix opacity-75">'
              html+='<div class="col-md-5">'
                html+='<span class="glyphicon glyphicon-phone text-sm"></span> &nbsp;<a href="#" data-toggle="modal" id="formMod"> '+ msg[i] + '</a>'
              html+='</div>'
              html+='<div class="col-md-7">'
                html+='<span class="glyphicon glyphicon-map-marker text-sm"></span> &nbsp;<a href="#" > Battery Level : '+batteries+ '</a>'
              html+='</div>'

            html+='</div'
            html+='<div class="clearfix">'
              html+='<div class="col-lg-12">'
                html+='<span class="opacity-75"><span class="fa fa-circle fa-fw text-success text-sm"></span>  &nbsp; <a href="#" >Online</a></span>'
              html+='</div>'
            html+='</div>'
